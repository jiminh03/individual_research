...
def check_falling(y_values):
    ...
    with open("fall_detection_log.txt", "a") as log_file:  # 텍스트 파일 열기
        if previous_y_values is not None and len(y_values) >= 6 and len(previous_y_values) >= 6:
            ...
            if (falling_threshold is not None) and (maximum - minimum <= falling_threshold):
                if (first_point_diff <= first_point_threshold) and (second_point_diff <= second_point_threshold):
                    if fallen_state:  # 상태가 이미 낙상으로 설정되어 있을 때는 기록하지 않음
                        log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - Laying down\n")
                        ...
                        fallen_state = True
                        log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - Fall Detected\n")
                        print("Fall Detected")
                else:
                    if fallen_state:
                        fallen_state = False
                        log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - Safe\n")
                        print("Safe")
            else:
                if fallen_state:  # 낙상이 감지되지 않은 경우에는 Safe 상태를 한 번만 기록
                    fallen_state = False
                    log_file.write(f"{time.strftime('%Y-%m-%d %H:%M:%S')} - Safe\n")
                    print("Safe")
...
results = model(source="/Users/jiminhwang/Downloads/fall-01-cam0.mp4", show=True, conf=0.3, stream=True, save=True)
